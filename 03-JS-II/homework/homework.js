...
function fizzBuzz(numero) {
  ...
  if (Number.isInteger(numero / 3)) return "fizz";
  if (Number.isInteger(numero / 5)) return "buzz";
  return numero;
}
...
function esPrimo(numero) {
  ...
  var contadorDivisiones = 0;

  if (numero == 0 || numero == 1) return false;
  else {
    for (var i = 1; i <= 100; i++) {
      if (Number.isInteger(numero / i)) contadorDivisiones++;
      if (contadorDivisiones > 2) return false;
    }
    if (contadorDivisiones == 2) return true;
  }
}
...
function tablaDelSeis() {
  ...
  for (var i = 0; i <= 10; i++) {
    arregloTabla[i] = 6 * i;
    console.log("6 X " + i + " = " + arregloTabla[i])
  }
  return arregloTabla;
}
